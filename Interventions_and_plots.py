...
catchment_area_new_traditional = 10000000  # New catchment area after traditional intervention in m^2
...
Waterpump_capacity_stagewise.loc[:, 15:] = waterpumpcapacity_new
...
catchment_area_stagewise.loc[:, 1:] = catchment_area_new_traditional
# flexible intervention
...
# stagewise intervention
Waterleakage_stagewise = pd.DataFrame([[leakage_zero + (10 * x) for x in range(i)]], index=[1], columns=np.arange(1, i + 1))
for year in range(20, i + 1):
    Waterleakage_stagewise.loc[1, year] = 10 * (year - 20)  # Set to 0 at fix_year and increase by 10 each year after
# flexible intervention
...
Cost_stagewise[1] = cost_catchment_area_increase * (catchment_area_new_traditional - catchment_area_zero)
Cost_stagewise[15] = cost_water_pump_capacity_increase
Cost_stagewise[20] = cost_fixing_leakage
# flexible intervention
...
total_costs2, intervention_costs2, env_costs2, unmet_demand_costs2, _, _, _, avg_water_currently2, avg_leakage_traditional = monte_carlo_total_cost(
    1000, 50, pop_df, rainfall_cdf_df, Waterpump_capacity_traditional, Waterleakage_traditional, Cost_traditional, catchment_area_traditional
)
total_costs3, intervention_costs3, env_costs3, unmet_demand_costs3, _, _, _, avg_water_currently3, avg_leakage_stagewise = monte_carlo_total_cost(
    1000, 50, pop_df, rainfall_cdf_df, Waterpump_capacity_stagewise, Waterleakage_stagewise, Cost_stagewise, catchment_area_stagewise
)
total_costs4, intervention_costs4, env_costs4, unmet_demand_costs4, _, _, _, avg_water_currently4, avg_leakage_flexible = monte_carlo_total_cost(
    1000, 50, pop_df, rainfall_cdf_df, Waterpump_capacity_flexible, Waterleakage_flexible, Cost_flexible, catchment_area_flexible
)
